...
export function SettingsPage({ configPath, metadata, onConfigPathChange }: SettingsPageProps) {
  ...
  const exportConfig = async () => { try { const result = await exportAmkrConfig(configPath); setTransfer(JSON.stringify(result.config, null, 2)); setRevision(result.config_revision); setNotice("已导出可迁移配置。"); setError(null); } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); } };
  const importConfig = async () => { if (!revision || !window.confirm("导入将替换供应商与路由配置。是否继续？")) return; try { const config: unknown = JSON.parse(transfer); const result = await importAmkrConfig(revision, config, configPath); setRevision(result.config_revision); setNotice("配置已导入，AMKR 已热重载。"); setError(null); } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); } };
  return <section className="settings-page" aria-labelledby="settings-heading">
    ...
    <form className="config-path-form" onSubmit={(event) => { event.preventDefault(); onConfigPathChange(draftConfigPath.trim() || null); }}>
      <label>配置路径<input value={draftConfigPath} onChange={(event) => setDraftConfigPath(event.target.value)} placeholder="留空使用默认 AMKR 配置" /></label>
      <button type="submit">使用配置</button>
    </form>
