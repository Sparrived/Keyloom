...
describe("RoutingPage", () => {
  beforeEach(() => {
    ...
    invokeMock.mockResolvedValue({
      ...
      routes: [{ id: "model-a", aliases: ["alias-a"], routing_mode: "round_robin", targets: [{ provider: "provider-a", key: "key-a", upstream_model: "model-a" }, { provider: "provider-b", key: "key-b", upstream_model: "model-b" }] }],
    });
  });

  it("shows only the selected model route panel", async () => {
    const routes = [
      { id: "model-a", aliases: ["alias-a"], routing_mode: "round_robin", targets: [{ provider: "provider-a", key: "key-a", upstream_model: "model-a" }] },
      { id: "model-b", aliases: ["alias-b"], routing_mode: "priority", targets: [{ provider: "provider-b", key: "key-b", upstream_model: "model-b" }] },
    ];
  ...
  it("does not expose model or upstream target configuration", async () => {
    ...
    expect(screen.getByText("provider-a / key-a / model-a")).toBeInTheDocument();
  ...
  it("updates only aliases and routing mode", async () => {
    ...
    await waitFor(() => expect(invokeMock).toHaveBeenCalledWith("update_amkr_route", {
      ...
      routeId: "model-a",
      targets: [
        { provider: "provider-a", key: "key-a", upstream_model: "model-a" },
        { provider: "provider-b", key: "key-b", upstream_model: "model-b" },
      ],
      aliases: ["alias-b"],
  ...
  it("saves displayed upstream target order when the drop preview has moved the source row", async () => {
    ...
    expect(targetList.querySelectorAll("li")[0]).toHaveTextContent("provider-b / key-b / model-b");
  ...
  it("returns the target list to its original order when dragging is cancelled", async () => {
    ...
    await waitFor(() => expect(targetList.querySelectorAll("li")[0]).toHaveTextContent("provider-a / key-a / model-a"));
  });
});
