...
**状态：功能实现完成，待签名发布与 Windows 10/11 实机验收**  
**日期：2026-07-12**  
**实现核验：2026-07-14**  
...
正式安装包只包含 Tauri 宿主和前端资源。Keyloom 使用现有 AMKR CLI；缺失时优先通过 `uv tool` 安装，没有 uv 时使用 `pipx`。

Windows 现代版本通常自带 WebView2；安装器检测缺失状态，按网络可用性提供自动安装或明确引导。AMKR 由 `uv tool`（优先）或 `pipx` 管理，Keyloom 不携带 Python，也不修改系统 Python。

建议目录：

```text
%LOCALAPPDATA%\Programs\Keyloom\
%APPDATA%\uv\tools\auto-model-key-router\  # 或 pipx 管理的工具目录
